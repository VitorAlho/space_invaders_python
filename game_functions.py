...
def create_fleet(ai_settings,screen,ship,aliens):
    ...
    # O espaçamento entre alienígenas é igual à largura de um alienígena
    alien = Alien(ai_settings, screen)
...
def check_events(ai_settings,stats,sb,play_button,screen,ship,aliens,bullets):
    """Responde a eventos de pressioamento de teclas e de mouse"""
...
def update_screen(ai_settings, stats, screen, sb, ship, aliens, bullets, play_button):
    ...
    if not stats.game_active:
        ...
        # Redesenha a espaçonave
        ship.blitme()		
        
        # Redesenha o alienígena
        aliens.draw(screen)
...
def check_bullet_alien_collisions(ai_settings,screen,stats,sb,ship,aliens,bullets):
    ...
    if len(aliens) == 0:
        # Destrói os projéteis existentes, aumenta a velocidade
        # do jogo, cria uma nova frota e aumenta o nivel de dificuldade
        bullets.empty()
...
def update_aliens(ai_settings,stats,screen,sb,ship,aliens,bullets):
    ...
    # Verifica se houveram colisões entre alienígenas e a espaçonave
    if pygame.sprite.spritecollideany(ship,aliens):
        ship_hit(ai_settings,stats,screen,sb,ship,aliens,bullets)
            
def fire_bullet(ai_settings,screen,ship,bullets):
    """Dispara um projétil se o limite máximo não foi atingido"""
